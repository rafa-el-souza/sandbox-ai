...
The system SHALL declare a module-level `IMAGE_DIGESTS: dict[str, str]` in `hydration.py` containing SHA256 digest references for all container images. The dict SHALL include entries for `wolfi_base`, `debian_trixie`, `squid`, `coredns`, and `postgres`. A comment above the dict SHALL document the digest rotation procedure using `docker manifest inspect`.

#### Scenario: IMAGE_DIGESTS contains all 5 images
- **WHEN** `IMAGE_DIGESTS` in `hydration.py` is inspected
- **THEN** it contains keys `wolfi_base`, `debian_trixie`, `squid`, `coredns`, and `postgres`, each with a value in the format `<registry>/<image>@sha256:<64-hex-chars>`
...
The system SHALL use `IMAGE_DIGESTS` values directly in `build_jinja_context()` for sandbox infrastructure images (proxy, dns) that are NOT user-configurable.
