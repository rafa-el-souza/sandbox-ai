...
- **THEN** none exist in helper-related code paths (the crossing is owned by `build_invocation`, which prefixes `sudo_pipe_cmd` per separate-user mode, or runs locally under operator-rootless)
