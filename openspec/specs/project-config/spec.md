...
This specification defines the `sandbox-ai.toml` project-wide configuration file — schema, location, Pydantic model, loader interface, and the centralized machinectl command prefix builder that consumes it.

## Requirements

### Requirement: Project Config File Location and Format
The system SHALL read project-wide configuration from a file named `sandbox-ai.toml` located in the project root directory. The file SHALL use TOML format with a `[host]` section.

#### Scenario: Valid project config parsed
- **WHEN** `sandbox-ai.toml` exists in the project directory with a valid `[host]` section
- **THEN** the system parses it into a `ProjectConfig` Pydantic model without errors

#### Scenario: Project config not found
...
The `ProjectConfig.from_toml()` class method SHALL accept an explicit `project_dir: str` parameter and SHALL NOT read `os.getcwd()` internally.

#### Scenario: Loader uses provided path
- **WHEN** `ProjectConfig.from_toml("/some/project")` is called
- **THEN** the loader reads `/some/project/sandbox-ai.toml` regardless of the process CWD

### Requirement: Pydantic Model Structure
The project config model SHALL use a `MachinectlAuth` StrEnum for the authentication field and a nested `HostConfig` model for the `[host]` section.
...
#### Scenario: HostConfig nested model
- **WHEN** a `ProjectConfig` is loaded
- **THEN** `project_config.host` is a `HostConfig` instance with `docker_unprivileged_user` and `machinectl_authentication` attributes
...
The `ProjectConfig` model, `MachinectlAuth` enum, `HostConfig` model, and `machinectl_cmd` function SHALL reside in `core/project_config.py`.

#### Scenario: Import path
- **WHEN** other modules need project config or machinectl prefix building
- **THEN** they import from `core.project_config`
