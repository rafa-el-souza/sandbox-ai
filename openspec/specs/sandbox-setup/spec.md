...
- **L6 daemon.json reserved key**: probe parses the existing daemon.json (if present) and deep-equals our `runtimes["sandbox-ai-runsc"]` value against the expected (path = `/usr/local/libexec/sandbox-ai/runsc`; runtimeArgs = exactly what `core.setup.l6_daemon_json.EXPECTED_RUNTIME` configures — single-sourced from that code constant, NOT a literal pinned in this spec). At present `EXPECTED_RUNTIME["runtimeArgs"]` is illustratively `["--oci-seccomp", "--ignore-cgroups"]`, but the normative expectation is "whatever `EXPECTED_RUNTIME` defines" so this spec does not drift when the arg set changes. **`--ignore-cgroups` is required under a rootless daemon: runsc cannot create its per-container systemd cgroup scope rootless — it reaches the *system* D-Bus and is auth-denied ("Interactive authentication required") — so it is told to skip cgroup setup entirely. The consequence is that OCI cpu/memory limits are render-time-only (NOT runtime-enforced) for gVisor containers. This is a deliberate, upstream-gated limitation (it would require gVisor to support rootless systemd-cgroup), NOT a temporary workaround; see `SECURITY.md` and the `cli-doctor` instance memory over-commit advisory.** **The probe is runtime-aware (F-023): a file deep-equal is necessary but NOT sufficient — the probe additionally confirms the daemon has *loaded* the runtime (the reserved key appears in `docker info` runtimes). File-correct AND loaded → skip; file-correct but NOT loaded → DRIFT (restart needed); mismatch or absence → merge.** Act always (re)starts when it runs (StartLimit-safe per the phase-order description), since a file-only deep-equal that skipped the restart left the runtime unregistered — the masking gap F-023 captured. The merge never touches the operator's other `runtimes["..."]` keys.
...
- **WHEN** the existing daemon.json contains `runtimes["sandbox-ai-runsc"]` matching the expected value (path + the current `EXPECTED_RUNTIME["runtimeArgs"]`, e.g. `["--oci-seccomp", "--ignore-cgroups"]`) AND `runtimes["runsc-debug"]` (operator's own runtime), AND the daemon has loaded the reserved runtime (it appears in `docker info` runtimes)
