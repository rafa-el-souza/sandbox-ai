...
The system SHALL allocate five consecutive `/24` subnets per sandbox instance from the `10.100.0.0–10.255.255.0` address space, with the allocation ledger mapping **instance name** (the registry key per `instance-registry`) to a reusable `base_index` integer.

#### Scenario: Five /24 Subnet Allocation
- **WHEN** a new sandbox instance requires IPAM allocation
- **THEN** the system assigns the lowest available `base_index` (integer, 0–7986) and derives five subnets as: `ISOLATED_NET = 10.(100 + g//256).(g%256).0/24`, `CORE_PROXY_NET = 10.(100 + (g+1)//256).((g+1)%256).0/24`, `DNS_NET = 10.(100 + (g+2)//256).((g+2)%256).0/24`, `EGRESS_NET = 10.(100 + (g+3)//256).((g+3)%256).0/24`, `IPC_NET = 10.(100 + (g+4)//256).((g+4)%256).0/24` where `g = base_index * 5`
...
- **THEN** the previously assigned `base_index` is reused and the quintuple of subnets is re-derived without modifying the ledger
...
- **THEN** each receives a distinct `base_index` and non-overlapping `/24` subnet quintuples; their daemon-side compose project names (`<sanitized-dev-username>-<inst-a>` and `<sanitized-dev-username>-<inst-b>` per `instance-registry`'s "Compose Project Name Prefix" requirement) are also distinct, preventing Docker network and volume name collisions
...
The system SHALL detect when all 7,987 concurrent slots are in use and abort allocation with an actionable error message.

#### Scenario: Exhausted ledger raises error
- **WHEN** all base_index values 0–7986 are in use in `ipam.json` and a new allocation is attempted
...
- **WHEN** `derive_static_ips(base_index)` is called
- **THEN** the returned dict includes `dnsdist_isolated_ip` as `<isolated_base>.56` and `dnsdist_dns_ip` as `<dns_base>.56`

#### Scenario: coredns IPs derived from base_index
- **WHEN** `derive_static_ips(base_index)` is called
- **THEN** the returned dict includes `coredns_dns_ip` as `<dns_base>.53` and `coredns_egress_ip` as `<egress_base>.53`

#### Scenario: db-postgres IPs derived from base_index
- **WHEN** `derive_static_ips(base_index)` is called
- **THEN** the returned dict includes `db_postgres_ip` as `<isolated_base>.54`
...
- **WHEN** the rendered `compose.yml` is inspected
- **THEN** the admin service is on exactly `ipc_net`
...
- **WHEN** the rendered `compose.yml` is inspected
- **THEN** the proxy service is on exactly `core_proxy_net` and `egress_net`

#### Scenario: coredns network membership unchanged
- **WHEN** the rendered `compose.yml` is inspected
- **THEN** the coredns service is on exactly `dns_net` and `egress_net`

#### Scenario: dnsdist network membership unchanged
- **WHEN** the rendered `compose.yml` is inspected
- **THEN** the dnsdist service is on exactly `isolated_net` and `dns_net`
...
- **THEN** the db-postgres service is on exactly `isolated_net`

#### Scenario: New networks are internal
- **WHEN** the rendered `compose.yml` is inspected
- **THEN** `core_proxy_net` and `dns_net` both have `internal: true`, `enable_ipv6: false`, and `com.docker.network.bridge.enable_ip_masquerade: 'false'`
...
- **WHEN** the rendered `compose.yml` is inspected
- **THEN** the core service's `NO_PROXY` includes `{{ isolated_subnet }}`, `{{ core_proxy_subnet }}`, and `{{ ipc_subnet }}`

#### Scenario: Firecrawl NO_PROXY scoped to its networks
- **WHEN** the rendered `mcp-firecrawl.yml` is inspected
- **THEN** the firecrawl service's `NO_PROXY` includes `{{ core_proxy_subnet }}` and `{{ dns_subnet }}` but does NOT include `{{ isolated_subnet }}`
