...
- **THEN** it contains entries for `dns-sidecar/Corefile`, `proxy/squid.conf`, `core/.gitconfig`, `core/.npmrc`, `core/.bashrc`, `core/CLAUDE.md`, `admin/.zshrc`, and `admin/.tmux.conf`
...
- **THEN** `_STATIC_CONFIG_CORE` contains only `.claude.json`, `_STATIC_CONFIG_ADMIN` contains only `gitmux.conf` and `starship.toml`, and `_STATIC_CONFIG_PROXY` contains only `ERR_SANDBOX_403`
