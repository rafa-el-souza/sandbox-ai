...
## Derived acceptance batteries (generated, never committed)

A baseline JSON is the **frozen pre-fix yardstick** — never hand-edited (a change's "the cell flips ✅"
claim is meaningless if you can edit the cell). Acceptance batteries that layer a change's deltas on top
of a baseline are therefore **generated by a builder script that derives a copy** — the baseline is read,
never mutated:
...
`sandbox start` (the F-018/F-055 empty-crossing class) — a false `START_CORE_RUNNING_FAIL` on the apt
family even though the stack is up. A harness must verify across the boundary with the *reliable* primitive
(`sudo -u … env XDG_RUNTIME_DIR DOCKER_HOST docker …`, non-PTY), not the fragile one the product itself
moved off of (C-009: `machinectl shell` → `sudo systemd-run --pipe`).

The **builder is tracked** (single source of truth); the **generated battery is not** — emit it into the
gitignored run area and point `vmlease` at it:
