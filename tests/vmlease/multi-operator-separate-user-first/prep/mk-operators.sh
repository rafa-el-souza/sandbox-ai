# Create the extra operators for the multi-operator (separate-user first) arc.
#
# DRY refactor: replaces the per-operator mk-op2 / mk-op3f [[prep.setup]] steps
# with ONE loop over the operator list, the create-logic written ONCE. vmlease
# prep script= inlines this file's TEXT as the step command (no arg passing, no
# host-side callable), so the dedup MUST be an internal loop.
#
# This arc is a 3-operator file: operators = op2 op3f (NO op4 — there is no
# fused F-071 creation+setup+assert probe in this arc; see the battery header).
#
# Behavior-equivalence with the separate mk-opN steps it replaces:
#   - same operators created (op2 op3f), same NOPASSWD sudoers drop-in, same
#     enable-linger (+ rundir poll), same R6 source distribution
#     (/tmp/${op}-src.tar.gz cp + chmod 0644), same per-operator uv sync, and
#     the same op3f subid-strip.
#   - FAIL-FAST: the original mk-op2/mk-op3f were SEPARATE required=true steps,
#     so the first operator's failure hard-aborted before the next was created.
#     This loop replicates that: any operator sub-step failure makes the loop
#     exit non-zero (PREP_HARD_FAIL) WITHOUT creating the later operator.
OPERATORS="op2 op3f"
