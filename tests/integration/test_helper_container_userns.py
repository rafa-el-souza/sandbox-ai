...
def _grant_parent_access(parent: Path, daemon_user: str) -> None:
    """Apply effective + default ACLs so the daemon user can read/write inside ``parent``.

    Mirrors the production phase ordering: the cli's `_phase_acl_grant` plus
    `_phase_helper_*`'s default-ACL setup land before the helper runs.
    """
...
def test_helper_mkdir_chown_dirs_lands_host_absolute_ownership(tmp_path: Path) -> None:
...
def test_helper_chown_files_lands_host_absolute_ownership_and_mode(tmp_path: Path) -> None:
    daemon_user, auth = _check_preconditions()
    _grant_parent_access(tmp_path, daemon_user)
