...
class TestWorkspaceAdd:
    def test_no_flags_rejected(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
    ...
    def test_copy_without_equals_rejected(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
    ...
    def test_duplicate_name_across_flags_rejected(
        self,
        runner: CliRunner,
    ...
    def test_collision_with_existing_rejected(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
    ...
    def test_running_instance_rejected(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
    ...
    def test_backup_lock_held_rejected(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
    ...
    def test_happy_path_empty_workspace(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
        ...
        assert (user_home / "workspaces" / "foo" / "extra").is_dir()

    def test_happy_path_copy_workspace(
        self,
        runner: CliRunner,
        register: Callable[..., Path],
        user_home: Path,
    ...
    def test_lock_contention_rejected(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
...
class TestWorkspaceRemove:
    def test_mutex_backup_and_purge(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
    ...
    def test_non_tty_without_flags_rejected(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
        ...
        register("foo", workspaces=[("main", "empty", None), ("scratch", "empty", None)])
    ...
    def test_purge_happy_path(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app

        instance_dir = register("foo", workspaces=[("main", "empty", None), ("scratch", "empty", None)])
        scratch = user_home / "workspaces" / "foo" / "scratch"
    ...
    def test_tty_prompt_yes_runs_backup(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
    ...
    def test_tty_prompt_no_runs_purge(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
...
class TestWorkspaceRename:
    ...
    def test_old_not_found_rejected(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
    ...
    def test_new_collides_with_existing(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
    ...
    def test_happy_path(
        ...
        instance_dir = register("foo", workspaces=[("main", "empty", None)])
        old_path = user_home / "workspaces" / "foo" / "main"
        new_path = user_home / "workspaces" / "foo" / "primary"
    ...
    def test_other_oserror_propagates(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        import errno
...
def _make_backup(
    user_home: Path, inst: str, ws_name: str, ts: str, *, source_instance: str | None = None
) -> Path:
    """Create a finalized backup tree under ``<home>/workspaces/_backups/`` and
    return the backup directory."""
    src_inst = source_instance or inst
    target = user_home / "workspaces" / "_backups" / src_inst / ws_name / ts
...
class TestWorkspaceRestore:
    def test_unknown_dest_instance(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
    ...
    def test_dest_workspace_collision_rejected(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
    ...
    def test_no_backup_found_rejected(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
    ...
    def test_omitted_spec_picks_latest_unique_match(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app

        instance_dir = register("foo", workspaces=[("main", "empty", None)])
        _make_backup(user_home,"foo", "scratch", "2026-05-07-00-00-00")

        result = runner.invoke(app, ["workspace", "restore", "foo", "scratch"])
        assert result.exit_code == 0, result.output
        restored = user_home / "workspaces" / "foo" / "scratch"
        assert (restored / "data.txt").exists()
        toml_text = (instance_dir / "sandbox.toml").read_text()
        assert "[workspaces.scratch]" in toml_text
        assert 'bootstrap_mode = "copy"' in toml_text

    def test_omitted_spec_ambiguous_refuses(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app

        register("foo", workspaces=[("main", "empty", None)])
        _make_backup(user_home,"a", "scratch", "2026-05-07-00-00-00", source_instance="a")
        _make_backup(user_home,"b", "scratch", "2026-05-07-00-00-00", source_instance="b")

        result = runner.invoke(app, ["workspace", "restore", "foo", "scratch"])
        assert result.exit_code == 1
        assert "multiple source instances" in result.output.lower()

    def test_fully_qualified_spec(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app

        register("foo", workspaces=[("main", "empty", None)])
        _make_backup(user_home,"oldfoo", "scratch", "2026-05-07-00-00-00", source_instance="oldfoo")
    ...
    def test_invalid_dest_workspace_name(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
...
class TestWorkspaceList:
    ...
    def test_default_lists_live_and_backups(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app

        register("foo", workspaces=[("main", "empty", None)])
        _make_backup(user_home,"foo", "main", "2026-05-07-00-00-00")
    ...
    def test_no_backups_flag_suppresses_section(
        self, runner: CliRunner, register: Callable[..., Path], user_home: Path
    ) -> None:
        from cli.main import app
