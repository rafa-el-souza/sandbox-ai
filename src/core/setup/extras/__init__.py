...
discovery would pick them up: they live in this ``core.setup.extras``
...
Contract the Milestone-F extra modules MUST satisfy
-----------------------------------------------------

Each extra is a module ``core.setup.extras.<name>`` (created by Milestone F's
G8b — NOT by this change) that exports a single module-level
...
imports cleanly today, before Milestone F lands those two modules — the lazy
loader simply raises a clear :class:`ModuleNotFoundError` if asked to resolve
an extra whose module does not yet exist.
"""
...
class ExtraSpec:
    ...
    Attributes:
        ...
        module: The fully-qualified module that Milestone F's G8b creates and
            from which :meth:`load_phase` imports the ``PHASE`` object.
    """
    ...
    def load_phase(self) -> Phase:
        ...
        Lazy by design: this registry must import before Milestone F adds
        ``core.setup.extras.fapolicyd`` / ``core.setup.extras.aide``. The
        import is deferred to call time so a base-ceremony run that touches
        no extra never imports those modules, and a stale registry entry
        fails loudly (``ModuleNotFoundError`` / ``AttributeError``) rather
        than silently.
        """
