...
The SUDO sudoers rule enumerates each dispatcher op as a `Cmnd_Alias SANDBOX_OPS` entry (full `MACHINECTL_PATH shell <user>@.host /bin/bash -c <dispatch>\ <op>[\ *]` prefix, backslash-escaped whitespace, zero `"` chars per F-004, no `Digest_Spec` per F-003, rendered from `core.dispatch.Op`). POLKIT mode is coarser: `org.freedesktop.machine1.shell` cannot inspect argv, so the polkit rule is an **action-level** grant and per-op narrowing lives only at the application layer (the orchestrator only calls `core.dispatch`, convention-meta-test-enforced) — a deliberate, documented asymmetry, not a defect.
