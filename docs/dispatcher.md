...
3. **Add the target-argv builder** — pure function from validated args to the `["/bin/bash", "-c", "<inner>"]` (or hardened-`docker run`) argv. Reuse existing primitives (e.g. `core.helper_container.hardened_docker_run`) rather than re-deriving flag lists.
